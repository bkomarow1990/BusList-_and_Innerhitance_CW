...
public:
	...
	void eraseFront();
	void pushAfter(const T& searchData, const T& data);// âñòàâêà íîâîãî åëåìåíòà ï³ñëÿ çàäàíîãî
	void clear();
	...
	void deleteElemWithNumber(const size_t& number); //only for classes that have getter for size_t number
	void reverseList();
...
inline void FwdList<T>::deleteElemWithIndex(const size_t& index)
{
	...
	if (tmp == head && tmp->next == tail && tail!=nullptr)
	{
		head = tail;
		tail = nullptr;
	
	} 
	else if(index==0 && tmp->next!=nullptr){
